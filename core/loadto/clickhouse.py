...
class ClickhouseServices(object):
    ...
    def create_sui_tale_raws(self):
        ...
        err_create_table, message_reate_table = self.ch_server.execute_query(
            create_table_query
        )
    ...
    def insert_into_table(self, message):
        ...
        datas = [
            {
                "raw_datas": message.data.decode("utf-8")
                .replace("\\", "")
                .replace('"{', "{")
                .replace('}"', "}")
            }
        ...
        insert_datas_query = self.query.insert_data_to_table(
            self.db_name, self.tb_name, self.columns
        )

        err_insert_datas, message_insert_datas = ch_server.insert_data(
            insert_datas_query, datas
        )
